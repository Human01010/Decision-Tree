...
# Predict function with explicit type casting for consistency
def predict(tree, instance):
    if not isinstance(tree, dict):
        return int(tree)  # Ensure the return value is an integer
...
def cross_validate(data, features):
    ...
    for train_index, test_index in kf.split(data):
        ...
        # Ensure predictions and test data are integer-encoded
        predictions = [int(predict(tree, row)) for _, row in test_data.iterrows()]
        accuracies.append(accuracy_score(test_data.iloc[:, 0].astype(int), predictions))  # Ensure target is integer
